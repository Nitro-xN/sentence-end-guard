...
export default class EditorHandler {
  ...
  constructor(editor) {
    ...
    this.sentenceEndMarker = atom.config.get('sentence-end-guard-for-japanese.sentence-end-marker');
    this.warningDistance = atom.config.get('sentence-end-guard-for-japanese.warning-distance');
    this.cautionDistance = atom.config.get('sentence-end-guard-for-japanese.caution-distance');
  ...
  watchText() {
    if (atom.config.get('sentence-end-guard-for-japanese.enable-sentence-end-guard')==false) return;
    var counter = function(str,key){
        return str.split(key).length -1;
    }
    // カーソルを置いている行までを解析する
    ...
    if (text.endsWith(this.sentenceEndMarker)&&commas!=this.commasPrev) {
      for (var i = 0; i < Object.keys(this.json).length; i++) {
        if (text.endsWith(this.json[i].phraseName+this.sentenceEndMarker)) {
          ...
          if (index-lastIndex<=this.warningDistance&&lastIndex!=-1&&index!=lastIndex) {
            // if (!atom.config.get('sentence-end-guard-for-japanese.silent-mode')) atom.beep();
            var message = "Sentence-end \"" + this.json[i].phraseName + "\" duplicates in next sentence. ";
            atom.notifications.addError(message,{
              dismissable: !atom.config.get('sentence-end-guard-for-japanese.dismiss-warning'),
              detail: detail
            });
          } else if(index - lastIndex<=this.cautionDistance&&lastIndex!=-1&&index!=lastIndex) {
            var message = "Sentence-end \"" + this.json[i].phraseName + "\" duplicates in near sentence.";
            atom.notifications.addWarning(message,{
              dismissable: !atom.config.get('sentence-end-guard-for-japanese.dismiss-caution'),
              detail: detail
            });
          } else {
