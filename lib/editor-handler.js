...
export default class EditorHandler {
        ...
        for (var k = 0; k < Object.keys(this.json).length; k++) {
          if (text[j].endsWith(this.json[k].phraseName)) {
            ...
            if (index-lastIndex<=warningDistance&&lastIndex!=-1&&index!=lastIndex) {
              // deploy marker
              // const range = new Range([row,column-this.json[k].phraseName.length],[row,column]);
              // var displayMarker = DisplayMarkerLayer.markBufferRange(range,{invalidate:'never'});
              // this.editor.decorateMarker(displayMarker,{
              //   type: 'block',
              //   class: 'linter-error'
              // });
  ...
  watchText() {
    if (!atom.config.get('sentence-end-guard.general.enable-real-time-alanysis')) return;
    var counter = function(str,key){
        return str.split(key).length -1;
    }
    // カーソルを置いている行までを解析する
